...
public class MhuStatsDialog extends BaseDialog{
    ...
    public void buildSettings(Table table){
        ...
        table.table(Tex.buttonDisabled, t-> rebuild[0] = () -> {
            t.clear();
            t.left();
            t.table(ta ->{
                field(ta, "mhu-stats.statsFntScl", "@stat.displaysize", shouldRebuild, rebuild, 1.5f);
                field(ta, "mhu-stats.statsHeight", "@height", shouldRebuild, rebuild, 60f);
            }).row();

//            t.check("mhu-stats-showhidden",  statsShowHidden, b -> {
...
//            }).row();
            ...
            t.table(tz-> {
            tz.button("@clear", () ->ui.showConfirm("@settings.clear.confirm", MhuVars::clear)).pad(10f).minWidth(200f).row();
    ...
    public void buildItems(Table t){
        ...
        cont.table(head -> {
            head.table(z -> {
                z.center();
                z.add(title(bundle.get("stat.buildcost") + " " + bundle.get("wavemode.totals"))).width(tabWidth).get().clicked(() -> setSort(10));
                z.add(title(Core.bundle.get("stat.buildcost")  + " " + Core.bundle.get("mhu-teams-lost"))).width(tabWidth).get().clicked(() -> setSort(11));
                z.add(title(Core.bundle.get("rules.title.unit") + " " + Core.bundle.get("mhu-teams-lost"))).width(tabWidth).get().clicked(() -> setSort(12));
                z.add(title(Core.bundle.get("rules.title.unit") + " " + Core.bundle.get("wavemode.totals"))).width(tabWidth).get().clicked(() -> setSort(13));
        ...
        else if(sort == 10)  tracked.sort(s ->countItms(s.id, 0));
        else if(sort == 11)  tracked.sort(s ->countItms(s.id, 1));
        else if(sort == 12)  tracked.sort(s ->countItms(s.id, 2));
        else if(sort == 13)  tracked.sort(s ->countItms(s.id, 3));
        else tracked.sort(s -> s.id);
        if(reverse) tracked.reverse();
        for(Team team : tracked){
            int i = team.id;
    ...
    public int countItms(int id, int type){
        int cnt = 0;
        for(int i = 0; i < Vars.content.items().size; i++){
            if(trackedSpent[id][i][type] == null) continue;
            cnt = cnt + trackedSpent[id][i][type].amount;
        }
        return cnt;
    }


    public int countUnts(int id, boolean type){
        int cnt = 0;
        int[][] aa = type ? unitTotal : unitLost;
        for(int i = 0; i < Vars.content.units().size; i++) cnt += aa[id][i];
        return cnt;
    }
    ...
    public void buildUnits(Table t){
        ...
        for(int i = 0; i < Team.all.length; i++){
            ...
            for(int j = 0; j < Vars.content.units().size; j++){
                if(tracked.contains(Team.get(i)))break;
                if(unitTotal[i][j] + unitLost[i][j] >= 1 ) tracked.addUnique(Team.get(i));
            }
        }
        if(sort == 1) tracked.sort(s -> s.color.hue());
        else if(sort == 2)  tracked.sort(s ->s.color.saturation());
        else if(sort == 3)  tracked.sort(s ->s.color.sum());
        else if(sort == 12)  tracked.sort(s ->countUnts(s.id, true));
        else if(sort == 13)  tracked.sort(s ->countUnts(s.id, false));
        if(reverse) tracked.reverse();
        Seq<Table> tabs = new Seq<>();
        ...
        for(Team team : tracked){
            ...
            for(int j = 0; j < 2; j++){
                ...
                Table itms = new Table();
                int row = 0;
                for(int ic = 0; ic < Vars.content.units().size; ic++){
                    if(read[team.id][ic] <= 0) continue;
    ...
    public void buildButtons(){
        ...
        if(sort >= 3){
            ...
            if(sort == 5) sortTxt = Core.bundle.get("stat.buildcost") + " " + Core.bundle.get("mhu-teams-lost");
            ...
            if(sort == 11) sortTxt = Core.bundle.get("stat.buildcost") + " " + Core.bundle.get("mhu-teams-lost");
            if(sort == 12) sortTxt = Core.bundle.get("rules.title.unit") + " " + Core.bundle.get("mhu-teams-lost");
            if(sort == 13) sortTxt = Core.bundle.get("rules.title.unit") + " " + Core.bundle.get("wavemode.totals");
        }
        ...
        buttons.table(z ->{
            ...
            z.button(Core.bundle.get("mhu-teams-sort.base") + sortTxt, () -> {
                ...
                if(sort > 3) sort = 0;
                rebuild();
            }).size(180, 64f).tooltip(bundle.get("mhu-teams-sort.tip")).row();
